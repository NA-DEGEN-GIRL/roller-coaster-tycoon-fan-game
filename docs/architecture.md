...
- 대기줄 타일은 화살표 방향을 가진다. 빌드 패널의 `↑/→/↓/←` 버튼을 누르면 `Queue` 툴로 전환되며 설치 방향이 바뀐다. `Queue` 툴에서는 `R` 키로도 방향을 순환한다.
- 대기줄이 놀이기구 입구와 연결되면, 연결된 대기줄 컴포넌트의 화살표 방향은 입구 쪽으로 자동 재계산된다. 수동 방향은 입구와 아직 연결되지 않은 대기줄을 놓을 때의 초기 방향으로만 사용된다.
- 대기줄 펜스는 고정 직선 조각이 아니라 주변 연결을 보고 자동 재계산한다. 들어오는 대기줄 변, 나가는 변, tail에 붙은 일반 길 입장 변은 열고 나머지 변만 막아, 코너에서 펜스가 줄 진행 방향이나 일반 길 연결을 가로막지 않게 한다.
- 대기줄 방향은 화살표가 가리키는 다음 타일이나 입구만 유효한 front 연결로 인정한다.
- 입구와 맞닿은 대기줄 타일도 화살표가 입구를 향해야 놀이기구와 연결된 것으로 본다.
- 일반 길에서 대기줄로 들어갈 수 있는 지점은 tail 타일에 붙은 일반 길 중 하나다. 여러 일반 길이 tail에 닿아도 입장 edge는 하나만 선택한다. 우선순위는 대기줄 진행 방향의 반대쪽 길이고, 없으면 front 변을 제외한 옆 길 중 하나를 선택한다. 선택된 입장 edge에는 작은 진입 화살표를 표시한다.
...
`Path`와 `Queue` 툴은 마우스를 누른 채 타일 위로 드래그하면 연속 설치된다.

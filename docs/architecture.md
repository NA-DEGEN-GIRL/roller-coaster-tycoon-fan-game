...
- 손님은 일반 길에서 대기줄과 인접한 타일에 도착했을 때만 대기줄 입장을 시도한다.
...
- open 상태의 놀이기구 입구와 연결된 대기줄 끝에 도착하면 `waiting` 상태로 멈춘다.
- 대기줄 타일은 현재 3명까지 수용하며, 손님은 입구에 가까운 슬롯부터 줄 방향으로 배치되어 서로 겹치지 않는다.
- 대기줄 슬롯이 모두 차 있으면 손님은 탑승하지 않고 다른 경로로 이동한다.
- 놀이기구가 `loading` 상태일 때 대기 손님이 `riding` 상태로 전환된다.
